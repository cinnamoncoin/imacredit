...
const string strMessageMagic = "IMAcredit Signed Message:\n";
...
{
    // IMAcredit: IsDust() detection disabled, allows any valid dust to be relayed.
...
//    nSubsidy >>= (nHeight / 840000); // IMAcredit: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 256 * 128; // IMAcredit: 128 blocks
static const int64 nTargetSpacing = 256; // IMAcredit: 256 seconds, about 4.3 minutes
static const int nKGWInterval = 8; // Timewarp fix - retargets every 12 blocks
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
        ...
        for (int i = 0; i < 256 && pindex != NULL; i++)
        {
        ...
        if (nUpgraded > 0)
            printf("SetBestChain: %d of last 256 blocks above version %d\n", nUpgraded, CBlock::CURRENT_VERSION);
        if (nUpgraded > 256/2)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IMAcredit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // IMAcredit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'a';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0x02891cbe04cf1ee452878726bdd7ab4ae908305f482701801be01a3fc746222b");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "08/31/2014 Is the KKK Stepping up its recruitment efforts?  Mom was 89 on the 29th!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
string GetWarnings(string strFor)
{
    ...
    if (!CLIENT_VERSION_IS_RELEASE)
        strStatusBar = _("This is a beta software - use at your own risk - not recommended for mining or merchant applications");
    ...
    {
        nPriority = 2000;
        strStatusBar = strRPC = _("Warning: Displayed transactions may not be correct! You may need to upgrade, or cold sync your blockchain.");
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IMAcreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IMAcreditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IMAcreditMiner(CWallet *pwallet)
{
    printf("IMAcreditMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("imacredit-miner");
    ...
    try { loop {
        ...
        printf("Running IMAcreditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            if (GetTimeMillis() - nHPSTimerStart > 30000)  // average over 30 seconds
            {
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IMAcreditMiner, pwallet));
}
